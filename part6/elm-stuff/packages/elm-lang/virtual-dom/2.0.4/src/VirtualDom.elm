...
Notice that you must give the _property_ name, so we use `className` as it
...
{-| Create arbitrary HTML _attributes_. Maps onto JavaScript’s `setAttribute`
...
Notice that you must give the _attribute_ name, so we use `class` as it would
